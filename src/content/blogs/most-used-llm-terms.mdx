...
### 3. Max Tokens <span class="line-through"> (Context length)</span>

> 💡 Just a quick note: Max Tokens is not the same as context length. Read about context length below.
...
Context length is the maximum number of tokens that the model can process/remember at a time. All follow-up conversation tokens will be added into the context length and calculated togther.

> Ex. If the model context length is 128k tokens, you will be able to have longer conversations with the model, and the model will also remember the previous conversation.
